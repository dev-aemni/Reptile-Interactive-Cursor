...
const SETTINGS = {
    thickness: 1,       // Line thickness (Higher = chunkier, Lower = stick-figure)
    overallSize: 0.75,     // Scale of the creature (Higher = bigger creature overall)
    bodyLength: 5,      // Number of leg segments (Higher = longer body / more legs)
    tailLength: 100,    // Number of tail segments (Higher = longer tail)
    smoothness: 1,      // How smoothly it follows the mouse (0.01 to 1.0)
    speed: 1.5
};
...
// Setup helpers (examples)
  ...
  function setupSimple() {
    ...
    setInterval(function() {
      Input.mouse.x += (targetMouse.x - Input.mouse.x) * SETTINGS.smoothness;
      Input.mouse.y += (targetMouse.y - Input.mouse.y) * SETTINGS.smoothness;
  ...
  function setupTestSquid(size, legs) {
    ...
    for (var i = 0; i < legNum; i++) {
      ...
      var leg = new LegSystem(node, jointNum, size * 30 * SETTINGS.speed, critter);
    }
    setInterval(function() {
      Input.mouse.x += (targetMouse.x - Input.mouse.x) * SETTINGS.smoothness;
      Input.mouse.y += (targetMouse.y - Input.mouse.y) * SETTINGS.smoothness;
  ...
  function setupLizard(size, legs, tail) {
    ...
    for (var i = 0; i < legs; i++) {
      ...
      for (var ii = -1; ii <= 1; ii += 2) {
        ...
        new LegSystem(node, 3, s * 12 * SETTINGS.speed, critter, 4);
      }
    }
    //Tail
...
// create creature and a long chain of segments
  ...
  ctx.lineWidth = SETTINGS.thickness; // Added Thickness Variable
  
  setupLizard(
    SETTINGS.overallSize * 3.5, // Multiplied so default size "1" isn't microscopic
    SETTINGS.bodyLength, 
    SETTINGS.tailLength
  );
